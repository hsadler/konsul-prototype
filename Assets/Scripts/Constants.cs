
public static class Constants
{


    public const int GALAXY_SIZE = 3000;

    public const int LIGHT_SPEED = 100;

    public const int PLANETARY_SYSTEMS_COUNT = 300;
    public const int PLANETARY_SYSTEMS_MIN_PLANETS = 0;
    public const int PLANETARY_SYSTEMS_MAX_PLANETS = 10;

    public const int STAR_MIN_SIZE_RADIUS = 4;
    public const int STAR_MAX_SIZE_RADIUS = 8;
    public const int STAR_MIN_LUMINOSITY = 1;
    public const int STAR_MAX_LUMINOSITY = 4;

    public const float PLANET_MIN_SIZE_RADIUS = 0.5f;
    public const float PLANET_MAX_SIZE_RADIUS = 1.9f;
    public const int PLANET_MIN_ORBIT_RADIUS = 5;
    public const int PLANET_MAX_ORBIT_RADIUS = 30;
    public const int PLANET_MIN_ORBIT_SPEED = 1;
    public const int PLANET_MAX_ORBIT_SPEED = 10;


}
